...
class GaleryArtState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(FileSystem.exists(path))
		{
			...
			for (i in 0...leMods.length)
			{
				if(leMods.length > 1 && leMods[0].length > 0) {
					...
					{
						if(modSplit[1] == '1')
							pushModArtGaleryToList(modSplit[0]);
						else
		...
		for (i in 0...artStuff.length)
		{
			...
			if(isSelectable) {
				if(artStuff[i][5] != null)
				{
					Paths.currentModDirectory = artStuff[i][5];
				}

				var icon:AttachedSprite = new AttachedSprite('art/' + artStuff[i][1]);
	...
	function pushModArtGaleryToList(folder:String)
	{
		if(modsAdded.contains(folder)) return;

		var artFile:String = null;
		if(folder != null && folder.trim().length > 0) artFile = Paths.mods(folder + '/data/art.txt');
		else artFile = Paths.mods('data/art.txt');

		if (FileSystem.exists(artFile))
		{
			var firstarray:Array<String> = File.getContent(artFile).split('\n');
			for(i in firstarray)
			{
