...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'starved': //???
                var bg:BGSprite = new BGSprite('covers/stcity', -820, -550, 0.7, 0.7);
                add(bg);

                var stbg:BGSprite = new BGSprite('covers/stbg', -820, -550, 1, 1);
                stbg.setGraphicSize(Std.int(stbg.width * 1.1));
                stbg.updateHitbox();
                add(stbg);

                var nateanddaikifuckingdead:BGSprite = new BGSprite('covers/nateanddaikifuckingdead', -820, -550, 1, 1);
			...
			case 'mono': //Monochrome
                var bg:BGSprite = new BGSprite('nothinglol', -200, -150, 1, 1);
                add(bg);		
		}
		...
		timeBar = new FlxBar(timeBarBG.x + -290, timeBarBG.y + 4, LEFT_TO_RIGHT, Std.int(timeBarBG.width - 10), Std.int(timeBarBG.height - 8), this,
			'songPercent', 0, 1);
		timeBar.scrollFactor.set();
		timeBar.createFilledBar(0xFF000000, 0xE3FFFFFF);
		...
		switch(eventName) {
			case 'Hey!':
				...
				switch(value1.toLowerCase().trim()) {
					case 'ncfbf' | 'boyfriend' | '0':
						value = 0;
					case 'ncfgf' | 'girlfriend' | '1':
						value = 1;
				}
				...
				if(value != 0) {
					if(dad.curCharacter.startsWith('ncfgf')) { //Tutorial GF is actually Dad! The GF is an imposter!! ding ding ding ding ding ding ding, dindinding, end my suffering
